<template>
  <div class="flex justify-between px-4 mt-5 sm:px-8 pt-4">
    <div class="flex items-center space-x-1 text-xs text-base">
      ...
      <span class="text-gray-600">Kategori Berita</span>
    </div>
            ...
            <tr v-for="(data, index) in kat" :key="data.id">
             ...
             <td class="p-2 ">{{data.nama_kategori}} </td>
              <td class="p-2  text-right ">
                <router-link :to="{ name:'tim-detail', params: { id:data.id}}"><button class="bg-gray-200 hover:bg-blue-300 text-gray-600 font-normal py-2 px-2 mr-1 rounded">
...
export default {
  ...
  data() {
    return {
      kat:"",
      form:{
        'nama_kategori' :'ashajshja'
      }
    }
  },
  methods:{
     getKategori(){
            StatistikService.getKategoriBerita()
            .then(response => {
           ...
           tambahkategori(){
            ...
            StatistikService.tambahKategoriBerita(this.form)
            .then(response => {
                console.log(response.data.data);
                this.$swal({
                  position: 'top-end',
            ...
            StatistikService.deleteKategoriBerita(id)
            .then(response => {
